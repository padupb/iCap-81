    ...
    "changes": [
      {
        ...
        "content_changes": [
          ...
          {
            "type": "replace",
            "old_text": "// Buscar pedidos com status \"Aguardando Aprovação\"\n      const result = await pool.query(`\n        SELECT DISTINCT\n          o.id,\n          o.order_id as \"orderId\",\n          o.product_id as \"productId\",\n          p.name as \"productName\",\n          u.abbreviation as \"unit\",\n          o.quantity,\n          o.supplier_id as \"supplierId\",\n          cs.name as \"supplierName\",\n          o.purchase_order_id as \"purchaseOrderId\",\n          oc.numero_ordem as \"purchaseOrderNumber\",\n          oc_company.name as \"purchaseOrderCompanyName\",\n          dest_company.name as \"destinationCompanyName\",\n          o.delivery_date as \"newDeliveryDate\",\n          o.created_at as \"requestDate\",\n          u_requester.name as \"requesterName\"\n        FROM orders o\n        LEFT JOIN products p ON o.product_id = p.id\n        LEFT JOIN units u ON p.unit_id = u.id\n        LEFT JOIN companies cs ON o.supplier_id = cs.id\n        LEFT JOIN ordens_compra oc ON o.purchase_order_id = oc.id\n        LEFT JOIN companies oc_company ON oc.empresa_id = oc_company.id\n        LEFT JOIN companies dest_company ON oc.cnpj = dest_company.cnpj\n        LEFT JOIN users u_requester ON o.user_id = u_requester.id\n        WHERE o.status = 'Aguardando Aprovação'\n        ORDER BY o.created_at DESC\n      `);",
            "new_text": "// Buscar pedidos com status \"Aguardando Aprovação\"\n      const result = await pool.query(`\n        SELECT DISTINCT\n          o.id,\n          o.order_id as \"orderId\",\n          o.product_id as \"productId\",\n          p.name as \"productName\",\n          u.abbreviation as \"unit\",\n          o.quantity,\n          o.supplier_id as \"supplierId\",\n          cs.name as \"supplierName\",\n          o.purchase_order_id as \"purchaseOrderId\",\n          oc.numero_ordem as \"purchaseOrderNumber\",\n          oc_company.name as \"purchaseOrderCompanyName\",\n          dest_company.name as \"destinationCompanyName\",\n          oc.cnpj as \"destinationCompanyCNPJ\",\n          o.delivery_date as \"newDeliveryDate\",\n          o.delivery_date as \"originalDeliveryDate\",\n          o.reprogramacao_justificativa as \"justification\",\n          o.created_at as \"requestDate\",\n          u_requester.name as \"requesterName\"\n        FROM orders o\n        LEFT JOIN products p ON o.product_id = p.id\n        LEFT JOIN units u ON p.unit_id = u.id\n        LEFT JOIN companies cs ON o.supplier_id = cs.id\n        LEFT JOIN ordens_compra oc ON o.purchase_order_id = oc.id\n        LEFT JOIN companies oc_company ON oc.empresa_id = oc_company.id\n        LEFT JOIN companies dest_company ON oc.cnpj = dest_company.cnpj\n        LEFT JOIN users u_requester ON o.user_id = u_requester.id\n        WHERE o.status = 'Aguardando Aprovação'\n        ORDER BY o.created_at DESC\n      `);",
            "start_offset": 2026,
            "end_offset": 2553
          }
        ]
    ...
    "changes": [
      {
        ...
        "content_changes": [
          ...
          },
          {
            "type": "replace",
            "old_text": "// Buscar pedidos com status \"Aguardando Aprovação\"\n      const result = await pool.query(`\n        SELECT DISTINCT\n          o.id,\n          o.order_id as \"orderId\",\n          o.product_id as \"productId\",\n          p.name as \"productName\",\n          u.abbreviation as \"unit\",\n          o.quantity,\n          o.supplier_id as \"supplierId\",\n          cs.name as \"supplierName\",\n          o.purchase_order_id as \"purchaseOrderId\",\n          oc.numero_ordem as \"purchaseOrderNumber\",\n          oc_company.name as \"purchaseOrderCompanyName\",\n          dest_company.name as \"destinationCompanyName\",\n          o.delivery_date as \"newDeliveryDate\",\n          o.created_at as \"requestDate\",\n          u_requester.name as \"requesterName\"\n        FROM orders o\n        LEFT JOIN products p ON o.product_id = p.id\n        LEFT JOIN units u ON p.unit_id = u.id\n        LEFT JOIN companies cs ON o.supplier_id = cs.id\n        LEFT JOIN ordens_compra oc ON o.purchase_order_id = oc.id\n        LEFT JOIN companies oc_company ON oc.empresa_id = oc_company.id\n        LEFT JOIN companies dest_company ON oc.cnpj = dest_company.cnpj\n        LEFT JOIN users u_requester ON o.user_id = u_requester.id\n        WHERE o.status = 'Aguardando Aprovação'\n        ORDER BY o.created_at DESC\n      `);",
            "new_text": "// Buscar pedidos com status \"Aguardando Aprovação\"\n      const result = await pool.query(`\n        SELECT DISTINCT\n          o.id,\n          o.order_id as \"orderId\",\n          o.product_id as \"productId\",\n          p.name as \"productName\",\n          u.abbreviation as \"unit\",\n          o.quantity,\n          o.supplier_id as \"supplierId\",\n          cs.name as \"supplierName\",\n          o.purchase_order_id as \"purchaseOrderId\",\n          oc.numero_ordem as \"purchaseOrderNumber\",\n          oc_company.name as \"purchaseOrderCompanyName\",\n          dest_company.name as \"destinationCompanyName\",\n          oc.cnpj as \"destinationCompanyCNPJ\",\n          o.delivery_date as \"newDeliveryDate\",\n          o.delivery_date as \"originalDeliveryDate\",\n          o.reprogramacao_justificativa as \"justification\",\n          o.created_at as \"requestDate\",\n          u_requester.name as \"requesterName\"\n        FROM orders o\n        LEFT JOIN products p ON o.product_id = p.id\n        LEFT JOIN units u ON p.unit_id = u.id\n        LEFT JOIN companies cs ON o.supplier_id = cs.This commit modifies the `/api/orders/reprogramacoes` route to correctly fetch orders with the status 'Aguardando Aprovação', including relevant join tables and adding fields for original delivery date and justification. It also adjusts access control logic for KeyUsers and approvers to ensure they see the correct reprogrammation requests.
...
Status: Teste em progresso...`;
  ...
  app.post("/api/pedidos/:id/reprogramar", isAuthenticated, async (req, res) => {
    try {
      ...
      const dataEntrega = convertToLocalDate(novaDataEntrega); // Conversão para fuso brasileiro
      ...
      const pedidoAntigo = pedidoResult.rows[0];

      // Criar um registro de reprogramação no log
      await storage.createLog({
        userId: req.user.id,
        action: "Reprogramou pedido",
        itemType: "order",
        itemId: pedidoId.toString(),
        details: `Pedido ${pedidoAntigo.order_id} (${pedidoAntigo.product_name}) reprogramado. Motivo: ${motivo}. Nova data: ${dataEntrega.toLocaleDateString('pt-BR')}.`
      });
      ...
      res.json({
        sucesso: true,
        mensagem: "Pedido reprogramado com sucesso e aguardando aprovação"
      });

    } catch (error) {
      console.error("❌ Erro ao reprogramar pedido:", error);
      res.status(500).json({
        sucesso: false,
        mensagem: "Erro ao reprogramar pedido"
      });
    }
  });
  ...
  return httpServer;
}companies oc_company ON oc.empresa_id = oc_company.id
        LEFT JOIN companies dest_company ON oc.cnpj = dest_company.cnpj
        LEFT JOIN users u_requester ON o.user_id = u_requester.id
        WHERE o.status = 'Aguardando Aprovação'
        ORDER BY o.created_at DESC
