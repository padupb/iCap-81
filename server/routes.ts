  ...
  if (objectStorageAvailable && objectStorage) {
    try {
      ...
      if (downloadTest && typeof downloadTest === 'object' && downloadTest.ok && downloadTest.value) {
        ...
        console.log(`✅ Array direto convertido para Uint8Array`);
...
Status: Teste em progresso...`;
      ...
      try {
        ...
        if (downloadedData && typeof downloadedData === 'object') {
          ...
          if (downloadedData.value) {
            log.log(`🔍 Tipo do value: ${typeof downloadedData.value}`);
            log.log(`🔍 Value é Buffer: ${downloadedData.value instanceof Buffer}`);
            log.log(`🔍 Value é Uint8Array: ${downloadedData.value instanceof Uint8Array}`);
            log.log(`🔍 Value é Array: ${Array.isArray(downloadedData.value)}`);
            log.log(`🔍 Tamanho do value: ${downloadedData.value?.length || 'indefinido'}`);
          }
        }
  ...
  // Configuração do multer para upload de PDF da ordem de compra
  const ordemCompraStorage = multer.diskStorage({
    destination: function (req, file, cb) {
      const uploadDir = path.join(process.cwd(), "uploads");
...
Here's the reasoning for the code combination:
The user intends to simplify the `server/routes.ts` file by removing redundant code and correcting syntax errors. The provided edited code snippet represents this simplified version. Therefore, the original file content should be entirely replaced by the edited code.
