...
sucesso: false,
  ...
  // ROTA: Buscar uma ordem de compra específica com seus itens
  app.get("/api/ordem-compra/:id", async (req, res) => {
    try {
      const { id } = req.params;

      console.log(`🔍 Buscando ordem de compra ID: ${id}`);
      ...
      const query = `
        SELECT 
          ...
          o.nome as obra_nome
        FROM ordens_compra oc
        LEFT JOIN companies e ON oc.empresa_id = e.id
        LEFT JOIN obras o ON oc.obra_id = o.id
        WHERE oc.id = $1
      `;
