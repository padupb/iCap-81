              ...
              try {
                ...
                if (isArrayLike && numericKeys.length > 10) {
                  ...
                  if (isValidByteArray) {
                    ...
                    console.log(`❌ Object array-like contém valores inválidos`);
                  }
                } else {
                  console.log(`❌ Object não é array-like válido ou muito pequeno`);
                }
              } catch (error) {
            ...
            for (const prop of ['data', 'content', 'bytes', 'buffer', 'body']) {
              if (directBytes[prop]) {
                ...
                if (propData instanceof Buffer) {
                  ...
                  console.log(`✅ Dados extraídos da propriedade '${prop}' como Uint8Array - ${finalBuffer.length} bytes`);
                  break;
                } else if (Array.isArray(propData) && propData.length > 10) {
                  const isValidByteArray = propData.every(v => typeof v === 'number' && v >= 0 && v <= 255);
                  if (isValidByteArray) {
                    finalBuffer = Buffer.from(propData);
                    console.log(`✅ Dados extraídos da propriedade '${prop}' como Array - ${finalBuffer.length} bytes`);
                    break;
                  }
                }
              }
      ...
      app.put("/api/ordem-compra/:id", isAuthenticated, async (req, res) => {
        try {
          ...
          if (req.user.id === 1 || req.user.isKeyUser === true || req.user.isDeveloper === true ||
            ...
            if (userCompany) {
              const companyCategory = await storage.getCompanyCategory(userCompany.categoryId);
              // Permitir edição se pode receber ordens OU tem permissão específica de edição
              hasEditPermission = companyCategory?.receivesPurchaseOrders === true ||
                                 companyCategory?.canEditPurchaseOrders === true;

              console.log(`🔍 Verificação de permissão para empresa ${userCompany.name}:`, {
                categoryId: userCompany.categoryId,
